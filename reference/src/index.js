...
function getOrCreateBulkPanel() {
	...
	bulkPanel.setAttribute(
		'aria-label',
		__( '一括編集', 'block-list-view-class' )
	);

	bulkPanel.innerHTML = `
		<div class="blvc-bulk-header">
			<span class="blvc-bulk-count"></span>
			<button class="blvc-bulk-deselect" type="button">${ __( '選択解除', 'block-list-view-class' ) }</button>
		</div>
		<div class="blvc-bulk-section">
			<span class="blvc-bulk-label">${ __( '名前の一括変更', 'block-list-view-class' ) }</span>
			<div class="blvc-bulk-row">
				<input type="text" class="blvc-bulk-name-input" placeholder="${ __( '新しいブロック名', 'block-list-view-class' ) }">
				<label class="blvc-bulk-number-label">
					<input type="checkbox" class="blvc-bulk-number-check">
					${ __( '連番', 'block-list-view-class' ) }
				</label>
				<button type="button" class="blvc-bulk-btn blvc-bulk-name-apply">${ __( '適用', 'block-list-view-class' ) }</button>
			</div>
		</div>
		<div class="blvc-bulk-section">
			<span class="blvc-bulk-label">${ __( 'クラスの一括編集', 'block-list-view-class' ) }</span>
			<div class="blvc-bulk-row">
				<input type="text" class="blvc-bulk-class-input" placeholder="${ __( 'クラス名', 'block-list-view-class' ) }">
				<select class="blvc-bulk-class-mode">
					<option value="add">${ __( '追加', 'block-list-view-class' ) }</option>
					<option value="replace">${ __( '置換', 'block-list-view-class' ) }</option>
					<option value="remove">${ __( '削除', 'block-list-view-class' ) }</option>
				</select>
				<button type="button" class="blvc-bulk-btn blvc-bulk-class-apply">${ __( '適用', 'block-list-view-class' ) }</button>
			</div>
		</div>
	`;
...
function updateBulkPanel() {
	...
	panel.hidden = false;
	panel.querySelector( '.blvc-bulk-count' ).textContent = __(
		`${ selectedIds.size } 件選択中`,
		'block-list-view-class'
	);
}
...
function enableNameEdit( leafEl, clientId ) {
	...
	btnEl.addEventListener( 'dblclick', ( e ) => {
		...
		nameInput.setAttribute(
			'aria-label',
			__( 'ブロック名を変更', 'block-list-view-class' )
		);
...
function injectClassInput( leafEl, clientId ) {
	...
	classInput.placeholder = __( '追加 CSS クラス', 'block-list-view-class' );
	classInput.setAttribute(
		'aria-label',
		__( '追加 CSS クラス', 'block-list-view-class' )
	);
...
function formatDomTreeNode( node, depth = 0 ) {
	if ( node.nodeType === Node.TEXT_NODE ) {
		...
		return text ? `${ '\t'.repeat( depth ) }${ escapeHtmlText( text ) }` : '';
	}
	...
	const classAttr = classStr ? ` class="${ escapeHtmlAttr( classStr ) }"` : '';
...
function getCustomClasses( clientId ) {
	const className = select( STORE ).getBlockAttributes( clientId )?.className || '';
...
function isEditorElement( el ) {
	const hasEditorClass = Array.from( el.classList ).some(
		( c ) => EDITOR_ELEMENT_CLASS_RE.test( c ) || EDITOR_ONLY_CLASSES.has( c )
	);
...
function getSel( el ) {
	...
	if ( clientId ) {
		...
		// 追加CSSクラスがあればそれを使用
		if ( custom.length ) return custom.map( ( c ) => `.${ c }` ).join( '' );
